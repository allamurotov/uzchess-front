    ...
    useEffect(() => {
        void (async () => {
            try {
                ...
                const rawNews = Array.isArray(nRes.data?.data) ? nRes.data.data : Array.isArray(nRes.data) ? nRes.data : [];
                setNews(rawNews.map((n: NewsItem & { createdAt?: string }) => ({ ...n, date: n.date || n.createdAt || '' })));
                setBooks(Array.isArray(bRes.data?.data) ? bRes.data.data : Array.isArray(bRes.data) ? bRes.data : []);
                const pRes = await api.get('/public/player').catch(() => null);
                if (pRes) {
