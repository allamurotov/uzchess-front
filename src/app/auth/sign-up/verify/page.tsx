    ...
    const submit = useCallback(async (e?: React.FormEvent) => {
        ...
        try {
            ...
            setTimeout(() => router.push(
                `/auth/sign-up/complete?phone=${encodeURIComponent(phone)}&code=${encodeURIComponent(otp)}`
            ), 700);
        } catch (err: unknown) {
            if (axios.isAxiosError(err)) {
                const msg = err.response?.data?.message ?? err.response?.data?.error;
                setError(msg ?? "Kod noto'g'ri. Qayta urinib ko'ring.");
            } else {
    ...
    return (
        <AuthCard>
            ...
            >
                ...
                <span className="text-sm font-medium">Telefon raqamni tasdiqlash</span>
            </button>
            ...
            <div
                className="inline-flex items-center gap-2 rounded-lg px-3 py-1.5 mb-4 bg-[#0D1117] border border-[#1F2937]">
                {isEmail
                    ? <svg className="w-4 h-4 flex-shrink-0 text-[#2EA6FF]" fill="none" viewBox="0 0 24 24"
                           stroke="currentColor">
