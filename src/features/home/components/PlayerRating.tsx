                ...
                {loading
                    ...
                    : players.map((p, idx) => (
                        <div key={p.id} className="flex items-center gap-2 py-1.5 border-b border-[#1A1A1A] last:border-0">
                            <span className="text-[#8A8F98] text-xs w-4 flex-shrink-0 font-medium">{idx + 1}</span>
                            <span className="text-white text-xs font-medium flex-1 truncate">{p.fullName}</span>
                            {p.country && (
                                <span className="text-[9px] text-[#8A8F98] bg-[#1A1D21] px-1.5 py-0.5 rounded flex-shrink-0">{p.country}</span>
                            )}
