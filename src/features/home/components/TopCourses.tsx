                ...
                {loading
                    ? Array.from({ length: 3 }).map((_, i) => (
                        <div key={i} className="flex gap-3">
                            <div className="w-14 h-14 bg-[#1A1D21] rounded-xl animate-pulse flex-shrink-0" />
                    ...
                    : list.length === 0
                        ...
                        : list.map((c) => (
                            <Link key={c.id} href={`/courses/${c.id}`} className="flex gap-3 group">
                                <div className="relative w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 bg-[#1A1D21]">
                                    <Image
