...
export default function LessonView({ course, lessons, currentIdx, onPrev, onNext, onSelect, courseId, isPurchased = false }: Props) {
    ...
    return (
        <div className="text-white">
            {/* Breadcrumb */}
            <div className="px-6 py-3 flex items-center gap-1.5 text-sm text-gray-400 border-b border-[#1A1A1A] flex-wrap">
                <Link href="/" className="text-[#2196F3] hover:underline">🏠 Asosiy</Link>
                <ChevronRight size={13} className="text-gray-600" />
                <Link href="/courses" className="hover:underline text-gray-400">O'rganish</Link>
                <ChevronRight size={13} className="text-gray-600" />
                <Link href={`/courses/${courseId}`} className="hover:underline text-gray-400 line-clamp-1">{course?.title}</Link>
            </div>
            ...
            {/* ── Locked lesson modal (Single.png) ── */}
            {showLocked && (
            ...
            {/* ── Get certificate modal (Get certificate.png) ── */}
            {showCert && (
